...
object ApplicationUtils {
     ...
     *
     * @param authorizationHeader The "Authorization" header string containing the JWT token.
     * @return A [SignedJWT] object representing the parsed JWT token.
     * @throws InvalidToken if the input is not a valid JWT token or the string is malformed.
     */
    fun getToken(request: HttpServletRequest): SignedJWT {
        val authorizationHeader = request.getHeader("Authorization")

        return getToken(authorizationHeader)
    }

    private fun getToken(authorizationHeader: String): SignedJWT {
        if (authorizationHeader == null || !authorizationHeader.startsWith("Bearer ")) {
            throw InvalidToken()
        }

        try {
            val jwtString = authorizationHeader.substring(7).trim()
